  ...
  std::vector< std::vector<double> > &dX)
{
  ...
  file.open(infn);
  {
    ...
    std::string junk; ssin >> junk; //first entry is junk
    while(true){
      double tmp_T;
      ssin >> tmp_T;
      if(!ssin.good()) break;
    ...
    while (std::getline(file, line)){
      ...
      while(true){
        ...
        ssin >> tmp_dX;
        if(!ssin.good()) break;
        if(tmp_dX==0) tmp_dX=1./0.; //XXX XXX XXX ??
...
int main(void){
  ...
  for(size_t i=0; i<file_list.size(); i++){
    ...
    if(tint_max[i]==0) tint_max[i] = 1.e15; //XXX
  ...
  of.precision(2);
  of<<"T\\tau_int ";
  for(size_t i=0; i<itint_list.size(); i++) of<<tint_list[itint_list[i]]<<" ";
  of<<"\n";
  of.precision(4);
